...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument('map', default_value=PathJoinSubstitution([pkg_path, 'maps', 'my_map.yaml'])))
    ...
    nav2 = IncludeLaunchDescription(
        ...
        launch_arguments={'use_sim_time': use_sim_time,
                          'map': map_yaml,
                          'params_file': PathJoinSubstitution([config_path, 'nav2_params.yaml'])}.items()
    )
    ld.add_action(nav2)
