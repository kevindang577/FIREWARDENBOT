#!/usr/bin/env python3
"""
Fire Detection Node for Fire Warden Bot
Uses multiple detection methods: color-based, heat signature, smoke detection, and deep learning
"""
...
class FireDetectionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('detection_method', 'combined')  # 'color', 'thermal', 'smoke', 'ml', 'combined'
        ...
        # Publishers
        self.fire_alert_pub = self.create_publisher(String, '/fire_detection/alert', 10)
        self.fire_location_pub = self.create_publisher(PointStamped, '/fire_detection/location', 10)
        self.fire_confidence_pub = self.create_publisher(Float32, '/fire_detection/confidence', 10)
        self.debug_image_pub = self.create_publisher(Image, '/fire_detection/debug_image', 10)
        self.fire_detected_pub = self.create_publisher(Bool, '/fire_detection/detected', 10)
        ...
        try:
            self.ml_detector = MLFireDetector()
            self.get_logger().info("ML fire detection model loaded successfully")
        except Exception as e:
            self.get_logger().warn(f"Could not load ML model: {e}")
        
        self.get_logger().info(f"Fire Detection Node initialized with method: {self.detection_method}")
    
    def camera_callback(self, msg: Image):
        """Process RGB camera image for fire detection"""
        try:
            self.rgb_image = self.bridge.imgmsg_to_cv2(msg, "bgr8")
            self.process_fire_detection()
        except Exception as e:
            self.get_logger().error(f"Error processing camera image: {e}")
    
    def thermal_callback(self, msg: Image):
        """Process thermal camera image for fire detection"""
        try:
    ...
    def process_fire_detection(self):
        """Main fire detection processing pipeline"""
        ...
        if self.detection_method in ['color', 'combined']:
            color_detections = self.detect_fire_by_color(self.rgb_image, roi_mask)
        ...
        if self.detection_method in ['thermal', 'combined'] and self.thermal_image is not None:
            thermal_detections = self.detect_fire_by_thermal(self.thermal_image, roi_mask)
            detections.extend(thermal_detections)
            debug_image = self.draw_detections(debug_image, thermal_detections, (0, 0, 255), "THERMAL")
        
        # Smoke detection
        if self.detection_method in ['smoke', 'combined']:
            smoke_detections = self.smoke_detector.detect_smoke(self.rgb_image, roi_mask)
            detections.extend(smoke_detections)
            debug_image = self.draw_detections(debug_image, smoke_detections, (255, 0, 0), "SMOKE")
        
        # ML-based detection
        if self.detection_method in ['ml', 'combined'] and self.ml_detector is not None:
            ml_detections = self.ml_detector.detect_fire(self.rgb_image, roi_mask)
        ...
        if final_detection:
            self.publish_fire_alert(final_detection, debug_image)
        else:
    ...
    def publish_fire_alert(self, detection: Dict, debug_image: np.ndarray):
...
class MLFireDetector:
    ...
    def detect_fire(self, image: np.ndarray, roi_mask: Optional[np.ndarray] = None) -> List[Dict]:
        """Detect fire using machine learning model"""
        detections = []
        ...
        # 1. Preprocess the image for your model
        # 2. Run inference
