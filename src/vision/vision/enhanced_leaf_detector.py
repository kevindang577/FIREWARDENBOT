#!/usr/bin/env python3
"""
Enhanced Leaf Detector with File Output for Visualization
Saves annotated images to files so you can see the bounding boxes
"""
...
class EnhancedLeafDetector(Node):
    def __init__(self):
        super().__init__('enhanced_leaf_detector')
        
        # Initialize CV Bridge
        self.bridge = CvBridge()
        
        # Create output directory
        self.output_dir = "/home/student/git/FIREWARDENBOT/detection_outputs"
        ...
        self.color_ranges = {
            'bright_orange': {
                ...
                'rgb_color': (0, 165, 255)  # BGR format for OpenCV
            },
            'red_orange': {
        ...
        self.min_contour_area = 100  # Lowered for better detection
        self.max_contour_area = 50000  # Increased for larger objects
        ...
        self.get_logger().info('Enhanced Leaf Detector Node initialized - Ready for foliage detection!')
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            self.get_logger().info(f'Processing frame {self.frame_count}, image size: {cv_image.shape}')
            
            # Always save the raw image first
            raw_filename = f"{self.output_dir}/frame_{self.frame_count:04d}_raw.jpg"
            ...
            annotated_filename = f"{self.output_dir}/frame_{self.frame_count:04d}_annotated.jpg"
            cv2.imwrite(annotated_filename, annotated_image)
            
            self.get_logger().info(f'Saved images: {raw_filename} and {annotated_filename}')
    ...
    def detect_leaves(self, image):
        ...
        # Also look for general orange/brown areas
        ...
        for color_name, color_info in self.color_ranges.items():
            ...
            for contour in contours:
                ...
                if self.min_contour_area < area < self.max_contour_area:
                    ...
                    # Draw on annotated image with thick lines
                    self.draw_detection(annotated_image, detection, color_info['rgb_color'])
        
        # Also detect any colored areas that might be leaves
        self.detect_general_colored_areas(image, annotated_image, detections)
    ...
    def detect_general_colored_areas(self, image, annotated_image, detections):
        """Detect any colored areas that might be autumn leaves"""
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        
        # Create a mask for any colorful areas (not just specific ranges)
        # Look for areas with decent saturation and brightness
        saturation_mask = cv2.inRange(hsv[:,:,1], 50, 255)  # Saturation
        brightness_mask = cv2.inRange(hsv[:,:,2], 50, 255)  # Value
        
        combined_mask = cv2.bitwise_and(saturation_mask, brightness_mask)
        ...
        for contour in contours:
            ...
            if 200 < area < 20000:  # Look for medium-sized colored areas
                x, y, w, h = cv2.boundingRect(contour)
                
                # Draw a different colored box for general detections
                cv2.rectangle(annotated_image, (x, y), (x + w, y + h), (255, 255, 0), 2)  # Cyan
                cv2.putText(annotated_image, "Colored Area", (x, y - 10), 
    ...
    def draw_detection(self, image, detection, color):
        """Draw detection visualization on the image with thick, visible lines"""
        ...
        cv2.rectangle(image, (x, y - label_size[1] - 15), 
...
def main(args=None):
    ...
    leaf_detector = EnhancedLeafDetector()
    
    try:
        rclpy.spin(leaf_detector)
    except KeyboardInterrupt:
        leaf_detector.get_logger().info('Enhanced Leaf Detector shutting down...')
    finally:
        ...
        leaf_detector.get_logger().info(f'Total frames processed: {leaf_detector.frame_count}')
        leaf_detector.get_logger().info(f'Images saved to: {leaf_detector.output_dir}')
